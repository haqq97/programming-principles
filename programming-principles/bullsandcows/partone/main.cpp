...
int main() {
	...
	int codeArray[1000]; //creating an empty array to load with user input
	int selectDigit = 0;
	int secretCode = 0;
	int numDigits = 0; 
	...
	if (selectDigit == 0) {//user1 inputs code manually
		...
		createRandomArray(codeArray, numDigits); //to create an array of random numbers of size numDigits
		cout << "Number to guess: ";
		printArray(codeArray, numDigits);
		cout << endl;
	}
